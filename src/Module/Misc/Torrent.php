...
class Torrent extends Handler
{
	...
	public function Analyze() {
		...
		if ($filesize > $this->max_torrent_filesize) {  //
			$this->error('File larger ('.number_format($filesize).' bytes) than $max_torrent_filesize ('.number_format($this->max_torrent_filesize).' bytes), increase JamesHeinrich\GetID3\Module\Misc\Torrent->max_torrent_filesize if needed');
			return false;
		}
