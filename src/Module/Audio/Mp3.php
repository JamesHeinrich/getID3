...
class Mp3 extends Handler
{
	...
	public function decodeMPEGaudioHeader($offset, &$info, $recursivesearch=true, $ScanAsCBR=false, $FastMPEGheaderScan=false) {
		...
		if ($MPEGaudioHeaderValidCache[$head4_key]) {
			...
			$this->warning('Invalid MPEG audio header ('.Utils::PrintHexBytes($head4).') at offset '.$offset);
			return false;
		}
	...
	public function RecursiveFrameScanning(&$offset, &$nextframetestoffset, $ScanAsCBR) {
		...
		for ($i = 0; $i < $this->mp3_valid_check_frames; $i++) {
			...
			if ($this->decodeMPEGaudioHeader($nextframetestoffset, $nextframetestarray, false)) {
				Utils::safe_inc($info['mp3_validity_check_bitrates'][intval($nextframetestarray['mpeg']['audio']['bitrate'])]);
				if ($ScanAsCBR) {
