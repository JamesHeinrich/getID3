...
class getid3_mpeg extends getid3_handler {
	...
	public function Analyze() {
		...
		$MPEGstreamData       = $this->fread(min(100000, $info['avdataend'] - $info['avdataoffset']));
		$MPEGstreamDataLength = strlen($MPEGstreamData);
		
		if (($VideoChunkOffset = strpos($MPEGstreamData, self::VIDEO_SEQUENCE_HEADER)) === false) {
			unset($info['fileformat']);
			return $this->error('Could not find start of video block in the first 100,000 bytes (or before end of file) - this might not be an MPEG-video file?');
		}
		...
		$VideoChunkOffset += strlen(self::VIDEO_SEQUENCE_HEADER);
		...
		$info['mpeg']['video']['pixel_aspect_ratio']      = self::videoAspectRatioLookup($info['mpeg']['video']['raw']['pixel_aspect_ratio']);
		$info['mpeg']['video']['pixel_aspect_ratio_text'] = self::videoAspectRatioTextLookup($info['mpeg']['video']['raw']['pixel_aspect_ratio']);
		$info['mpeg']['video']['frame_rate']              = self::videoFramerateLookup($info['mpeg']['video']['raw']['frame_rate']);
		...
		if (substr($MPEGstreamData, $VideoChunkOffset, 4) == self::VIDEO_EXTENSION_START) {
			$info['video']['codec'] = 'MPEG-2';
		} else {
			$info['video']['codec'] = 'MPEG-1';
		}

		if (($AudioChunkOffset = strpos($MPEGstreamData, self::AUDIO_START)) !== false) {
			...
			for ($i = 0; $i <= 7; $i++) {
				...
				if ($getid3_mp3->decodeMPEGaudioHeader(($AudioChunkOffset + 4) + 8 + $i, $getid3_temp->info, false)) {
